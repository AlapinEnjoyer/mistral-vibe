# Vaybeeton (Mistral Vaybeeton Fork)
...
```bash
uv pip install "git+https://github.com/AlapinEnjoyer/mistral-vibe"
```
...
- **Project-Aware Context**: Vaybeeton automatically scans your project's file structure and Git status to provide relevant context to the agent, improving its understanding of your codebase.
...
3. If this is your first time running Vaybeeton, it will:
...
Simply run `vbt` to enter the interactive chat loop.
...
You can start Vaybeeton with a prompt with the following command:

```bash
vbt "Refactor the main function in cli/main.py to be more modular."
```
...
You can run Vaybeeton non-interactively by piping input or using the `--prompt` flag. This is useful for scripting.

```bash
vbt --prompt "Refactor the main function in cli/main.py to be more modular."
```
...
Vaybeeton is configured via a `config.toml` file. It looks for this file first in `./.vibe/config.toml` and then falls back to `~/.vibe/config.toml`.

### API Key Configuration

Vaybeeton supports multiple ways to configure your API keys:

1. **Interactive Setup (Recommended for first-time users)**: When you run Vaybeeton for the first time or if your API key is missing, Vaybeeton will prompt you to enter it. The key will be securely saved to `~/.vibe/.env` for future sessions.
...
3. **`.env` File**: Create a `.env` file in `~/.vibe/` and add your API keys:
   ...
   Vaybeeton automatically loads API keys from `~/.vibe/.env` on startup. Environment variables take precedence over the `.env` file if both are set.

**Note**: The `.env` file is specifically for API keys and other provider credentials. General Vaybeeton configuration should be done in `config.toml`.
...
To use a custom agent, run Vaybeeton with the `--agent` flag:

```bash
vbt --agent my_custom_agent
```

Vaybeeton will look for a file named `my_custom_agent.toml` in the agents directory and apply its configuration.
...
You can configure MCP (Model Context Protocol) servers to extend Vaybeeton's capabilities. Add MCP server configurations under the `mcp_servers` section:
...
### Custom Vaybeeton Home Directory

By default, Vaybeeton stores its configuration in `~/.vibe/`. You can override this by setting the `VIBE_HOME` environment variable:

```bash
export VIBE_HOME="/path/to/custom/vibe/home"
```

This affects where Vaybeeton looks for:
